...
equils <- function(){
  ...
  if(any((x_star-xstar)>1e-10)){print("Unequal calculations in x")}
  if(any((y_star-ystar)>1e-10)){print("Unequal calculations in y")}
  return(list(xstary0=xstary0, ystarx0=ystarx0, xstar=x_star, ystar=y_star))
}
eq <- equils();eq
...
pdf("../../figures/vecField_lambda1.pdf",width=10)
par(mfrow=c(1,2),mar=c(2,1,1,1),oma=c(3,3,0,0))
lambda1<-2;vecField(x_vals,y_vals)
plot(-1:1,type='n',xlim=c(1,5),xlab="lambda1",ylab='')
for(lambda1 in seq(1,5,0.1)){
	points(lambda1,equils()$"xstar", pch=19, col=cols$'1'[1],cex=1.5)
	points(lambda1,equils()$"xstary0", pch=17, col=cols$'10'[2],cex=1.5)
	points(lambda1,equils()$"ystar", pch=19, col=cols$'2'[1],cex=1.5)
	points(lambda1,equils()$"ystarx0", pch=17, col=cols$'20'[2],cex=1.5)
}
abline(v=2,xpd=F)
dev.off()
